                ...
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700" wire:click="sortBy('starts_at')" style="user-select: none;">
                                Start Date {{ $sortBy === 'starts_at' ? ($sortDir === 'asc' ? '↑' : '↓') : '' }}
                            </th>
                            <th class="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider cursor-pointer hover:text-slate-700" wire:click="sortBy('ends_at')" style="user-select: none;">
                                End Date {{ $sortBy === 'ends_at' ? ($sortDir === 'asc' ? '↑' : '↓') : '' }}
                            </th>
                    ...
                    <tbody class="divide-y divide-slate-200">
                        @forelse($subscriptions as $sub)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-sm text-slate-500">{{ $sub->starts_at->format('M j, Y') }}</td>
                                <td class="px-4 py-3 text-sm text-slate-500">{{ $sub->ends_at?->format('M j, Y') ?? 'Ongoing' }}</td>
                                <td class="px-4 py-3 text-sm">
